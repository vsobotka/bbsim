...
def BBCalc(
        Trials=100000,
        # Number of trials to run through. More trials leads to more accurate results but longer compute times.
        ...
        GloriousEndurance=0,
        # The Bear's unique trait. Reduces damage by 5% each time you are hit, up to a 25% max reduction.
        ...
        TwoHander20=0,
        # Damage +20. Applies to the single target 2Hander attacks Cudgel (Mace), Pound (Flail), Smite (Hammer), Overhead Strike (Long/GreatSword).
        ...
        Flail2HPound=0,
        # Ignore +10% on body hits and +20% on headshots. Applies to 2H Flail Pound attack. Apply the +20 damage from Pound using the TwoHander20 switch.
        FlailMastery=0,
        # Flail Mastery. Will apply an extra 10% armor ignore on headshots for Pound. Does nothing unless Flail2HPound is set.
        ...
        DestroyArmorTwice=0,
        # Uses Destroy Armor two times instead of 1. Does nothing unless DestroyArmor or DestroyArmorMastery are set.
        ...
        Deathblow=0,
        # Qatal special. Ignore +20%. Damage x1.33. Assumes target is always setup for Deathblow value in calculation.
        Spearwall=0,
        # Warning: May take a long time to compute against durable targets, considering lowering number of trials.
        ...
        KillingFrenzy=0,
        Fearsome=0,
        # Will also return ,# of extra Fearsome checks, which are all attacks that deal 1-14 damage. Assign Attacker Resolve below.
        Atk_Resolve=50,
        # Only used if Fearsome is selected. 20% of (Resolve -10) is applied as a penalty to defender Resolve during morale checks.
        # Traits:
        ...
        Distracted=0,
        # Damage -35%. Set this if you want to simulate the attacker always being Distracted (applied by Nomads).
        ...
        while hp > 0:  # Continue looping until death.
            ...
            if head_roll <= Headshotchance:  # If headshot, do the following code blocks.
                ...
                if DArmorMod != 1:
                    ...
                    hp -= hp_roll
                    armor_roll = random.randint(Mind,
                                                Maxd) * ArmorMod * DArmorMod * GladMod * IndomMod * DamageMod * ExecMod
                ...
                # If not DestoryArmor, and no armor is present, apply damage directly to hp.
                    ...
                    hp_roll = hp_roll * NimbleMod * SkeletonMod * GladMod * IndomMod * (
                            (DamageMod * ExecMod * AimedShotMod) * DecapMod) * HeadMod
                ...
                # Otherwise, do the following.
                    ...
                    armor_roll = min(helmet, (
                            armor_roll * ForgeMod))  # Applying Forge, and armor damage cannot exceed current armor.
                    helmet -= armor_roll  # Armor damage applied to helmet.
                    # If the helmet does not get destroyed by the attack, do the following.
                    if helmet > 0:
                        hp_roll = max(0, (hp_roll * Ignore * NimbleMod * SkeletonMod * GladMod * IndomMod * (
                                (DamageMod * ExecMod * AimedShotMod) * DecapMod) - (helmet * 0.1)) * HeadMod)
                        if Hammer10 == 1:
                            hp_roll = max(hp_roll, 10)
                    ...
                    # If the helmet did get destoryed by the attack, do the following.
                        ...
                        OverflowDamage = max(0, (
                                hp_roll * (1 - Ignore) * NimbleMod * SkeletonMod * GladMod * IndomMod * (
                                (DamageMod * ExecMod * AimedShotMod) * DecapMod) - armor_roll))
                        hp_roll = (hp_roll * Ignore * NimbleMod * SkeletonMod * GladMod * IndomMod * (
                                (DamageMod * ExecMod * AimedShotMod) * DecapMod) + OverflowDamage) * HeadMod
                        if Hammer10 == 1:
                            hp_roll = max(hp_roll, 10)
                        hp = math.ceil(hp - hp_roll)
                ...
                if SplitMan == 1:
                    if BoneplateMod == 1:
                        ...
                        if body == 0:
                            ...
                            if body > 0:
                                SMhp_roll = max(0, (
                                        SMhp_roll * Ignore * NimbleMod * AdFurPadMod * GladMod * IndomMod * AttachMod - (
                                        body * 0.1)))
                                body = math.ceil(body)
                                hp = math.ceil(hp - SMhp_roll)
                            else:
                                OverflowDamage = max(0, (SMhp_roll * (
                                        1 - Ignore * AdFurPadMod) * NimbleMod * GladMod * IndomMod * AttachMod - SMarmor_roll))
                ...
                if BoneplateMod == 1:
                    ...
                    if DArmorMod != 1:
                        ...
                        hp_roll = hp_roll * NimbleMod * SkeletonMod * GladMod * IndomMod * AttachMod * (
                                (DamageMod * ExecMod * AimedShotMod) * DecapMod)
                        if Hammer10 == 1:
                            hp_roll = max(hp_roll, 10)
                        hp = math.ceil(hp - hp_roll)
                    else:
                        armor_roll = random.randint(Mind,
                                                    Maxd) * ArmorMod * GladMod * IndomMod * DamageMod * ExecMod * AttachMod
                        ForgeSaved += armor_roll - armor_roll * ForgeMod
                        armor_roll = min(body, (armor_roll * ForgeMod))
                        body -= armor_roll
                        if body > 0:
                            hp_roll = max(0, (
                                    hp_roll * Ignore * NimbleMod * SkeletonMod * AdFurPadMod * GladMod * IndomMod * AttachMod * (
                                    (DamageMod * ExecMod * AimedShotMod) * DecapMod) - (body * 0.1)))
                            if Hammer10 == 1:
                                hp_roll = max(hp_roll, 10)
                            body = math.ceil(body)
                            hp = math.ceil(hp - hp_roll)
                        else:
                            OverflowDamage = max(0, (hp_roll * (
                                    1 - Ignore * AdFurPadMod) * NimbleMod * SkeletonMod * GladMod * IndomMod * AttachMod * (
                                                             (
                                                                     DamageMod * ExecMod * AimedShotMod) * DecapMod) - armor_roll))
                            hp_roll = hp_roll * Ignore * NimbleMod * SkeletonMod * AdFurPadMod * GladMod * IndomMod * AttachMod * (
                                    (DamageMod * ExecMod * AimedShotMod) * DecapMod) + OverflowDamage
                            if Hammer10 == 1:
                                hp_roll = max(hp_roll, 10)
                            hp = math.ceil(hp - hp_roll)
                ...
                if SplitMan == 1:
                    ...
                    if helmet == 0:
                        ...
                        if helmet > 0:
                            ...
                            OverflowDamage = max(0, (
                                    SMhp_roll * (1 - Ignore) * NimbleMod * GladMod * IndomMod - SMarmor_roll))
            ...
            if (hp > 0 or NineLivesMod == 1) and Fleeing != 1:
                if Fearsome == 1:
                    if Flail3Head != 1 or (
                            Flail3Head == 1 and count % 3 == 1):  # Checking weapon and if 3Head only apply 1-14 Fearsome effect on every first hit.
                        if math.floor(hp_roll) > 0 and math.floor(hp_roll) < 15:
                            FearsomeProcs += 1
                        if math.floor(hp_roll) > 0:
                            MoraleChecks += 1
                            resolve_roll = random.randint(1,
                                                          100)  # Random number used to see if we pass/fail morale check.
                            chance_morale_drop = 100 - (((Def_Resolve * ResolveMod) - FearsomeMod) - 40 * (
                                    1 - hp / Def_HP))  # Formula for chance of losing morale from damage.
                            ...
                            if resolve_roll <= chance_morale_drop:  # If we fail the check, we will fall to Wavering, Breaking, or Fleeing, in that order.
                                if Wavering == 0:
                                    ...
                                    if Flail3Head == 1:
                                        ...
                                        hits_until_wavering.append(
                                            count)  # Return the time until Wavering for later data analysis.
                                elif Breaking == 0:
                                    Breaking = 1
                                    ResolveMod = .8
                                    if Flail3Head == 1:
                        ...
                        if Flail3Head == 1 and count % 3 != 1:  # This block is only run for 3Head's second/third hits.
                            if math.floor(hp_roll) >= 15:
                                ...
                                chance_morale_drop = 100 - (
                                        ((Def_Resolve * ResolveMod) - FearsomeMod) - 40 * (1 - hp / Def_HP))
                                if chance_morale_drop < 5:
    ...
    if Undead != 1 and Savant != 1:
        if len(hits_until_1st_injury) != 0:
            ...
            HitsToInjurePercent = [(i, HitsToInjureCounter[i] / len(hits_until_death) * 100) for i in
                                   HitsToInjureCounter]
        if len(hits_until_1st_heavy_injury_chance) != 0:
            ...
            HitsToHeavyInjuryChancePercent = [(i, HitsToHeavyInjuryChanceCounter[i] / len(hits_until_death) * 100) for i
                                              in HitsToHeavyInjuryChanceCounter]
        if len(hits_until_1st_morale) != 0:
            ...
            HitsToMoralePercent = [(i, HitsToMoraleCounter[i] / len(hits_until_death) * 100) for i in
                                   HitsToMoraleCounter]
        ...
        if len(hits_until_wavering) != 0:
            ...
            HitsToWaveringPercent = [(i, HitsToWaveringCounter[i] / len(hits_until_death) * 100) for i in
                                     HitsToWaveringCounter]
        if len(hits_until_breaking) != 0:
            ...
            HitsToBreakingPercent = [(i, HitsToBreakingCounter[i] / len(hits_until_death) * 100) for i in
                                     HitsToBreakingCounter]
        if len(hits_until_fleeing) != 0:
            ...
            HitsToFleeingPercent = [(i, HitsToFleeingCounter[i] / len(hits_until_death) * 100) for i in
                                    HitsToFleeingCounter]
        if Fearsome == 1:
